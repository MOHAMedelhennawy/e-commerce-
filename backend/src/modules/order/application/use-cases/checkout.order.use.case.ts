...
import type OrderInputDTO from "../dtos/order.intput.dto";
...
export default class CheckoutUseCase {
    ...
    async execute(dto: OrderInputDTO): Promise<void> {
        ...
        await this.uow.transaction(async (tx) => {
            await this.orderRepository.add(Order.create(userId, orderItems), tx);
            await this.productRepository.saveMany(products, tx);
            await this.cartRepository.save(existingCart, tx);
        });
    }
